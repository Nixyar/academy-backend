...
const JOB_PREVIEW_MIN_INTERVAL_MS = 5000;
...
const buildHtmlDocument = (job) => {
  if (!job) return '';
  const lang = job.outline?.lang || 'ru';
  const title = job.outline?.title || 'Site';
  const sectionsOrder = Array.isArray(job.sectionOrder) && job.sectionOrder.length
    ? job.sectionOrder
    : Object.keys(job.sections || {});
  const assembledSections = sectionsOrder.map((key) => job.sections?.[key] || '');
...
const persistJobPreview = async (job) => {
  if (!job || job.mode !== 'create') return false;
  if (!job.css) return false;
  const nowMs = Date.now();
  if (job.lastPreviewAt && nowMs - job.lastPreviewAt < JOB_PREVIEW_MIN_INTERVAL_MS) return false;

  const html = buildHtmlDocument(job);
  if (!html) return false;

  job.lastPreviewAt = nowMs;
  ...
  await mutateCourseProgress(job.userId, job.courseId, (progress) => {
    ...
    const baseResult =
      progress.result && typeof progress.result === 'object' && !Array.isArray(progress.result)
        ...
        : { html: null, meta: {} };
    const meta =
      baseResult.meta && typeof baseResult.meta === 'object' && !Array.isArray(baseResult.meta)
        ? { ...baseResult.meta }
        : {};
    return {
      ...progress,
      ...
      result: {
        ...
        meta: {
          ...meta,
          active_job_id: job.jobId,
          preview_updated_at: nowIso,
        },
      },
    };
  });
...
const startJobHeartbeat = (job) => {
  if (!job || job.heartbeatTimer) return;
  job.heartbeatTimer = setInterval(() => {
    heartbeatActiveJob(job, { status: job.status }).catch(() => { });
  }, JOB_HEARTBEAT_INTERVAL_MS);
};

const stopJobHeartbeat = (job) => {
  if (!job || !job.heartbeatTimer) return;
...
`;
  ...
  if (!job.css) {
    ...
    emit('css', { type: 'css', content: job.css, css: job.css });
    const previewSaved = await persistJobPreview(job);
    if (!previewSaved) await heartbeatActiveJob(job);
  }
  ...
  for (const [idx, key] of sectionsToGenerate.entries()) {
    emitStatus(
      job,
      'calling_llm',
      `генерация блоков ${idx + 1}/${sectionsToGenerate.length}`,
...
- If unsure, output a minimal valid <section> with heading + paragraph.\n`.trim()}`;
  ...
  const finalHtml = buildHtmlDocument(job);
  job.html = finalHtml;
...
const startJobRunner = (job, debugMode = false) => {
  ...
  void heartbeatActiveJob(job, { status: 'running' }).catch(() => { });
